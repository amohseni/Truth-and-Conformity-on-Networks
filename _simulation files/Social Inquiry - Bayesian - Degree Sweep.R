  ...
  # SIMULATIONS : PARAMETER SWEEP — DEGREE DENSITY
  ###########################################################################
  ...
  for (i in 1:numberOfInitialConditions) {
    ...
    for (j in 1:numberOfNetworkTypes) {
      ...
      for (k in 1:numberOfDegreeSettings) {
        ...
        if ( (RegDegreeSweep[k] * N) < 10) {
          write.csv(DeclarationDF, file = paste("results.Declarations.NetworkType=", NetworkTypeSweep[j], ".Degree=0", RegDegreeSweep[k]*N,".N=", N,".Init=", InitialDeclarationsSweep[i],".Runs=", numberOfSimulationsPerSetting, ".csv", sep=""))
          write.csv(BeliefDF, file = paste("results.Beliefs.NetworkType=", NetworkTypeSweep[j], ".Degree=0", RegDegreeSweep[k]*N,".N=", N,".Init=", InitialDeclarationsSweep[i],".Runs=", numberOfSimulationsPerSetting, ".csv", sep=""))
        } else {
          write.csv(DeclarationDF, file = paste("results.Declarations.NetworkType=", NetworkTypeSweep[j],".Degree=", RegDegreeSweep[k]*N,".N=", N, ".Init=", InitialDeclarationsSweep[i], ".Runs=", numberOfSimulationsPerSetting, ".csv", sep=""))
          write.csv(BeliefDF, file = paste("results.Beliefs.NetworkType=", NetworkTypeSweep[j], ".Degree=", RegDegreeSweep[k]*N, ".N=", N, ".Init=", InitialDeclarationsSweep[i], ".Runs=", numberOfSimulationsPerSetting, ".csv", sep=""))
        }
      }
    }
  }
