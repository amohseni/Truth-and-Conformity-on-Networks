...
NumberOfRounds <-
  ceiling(NumberOfTurns / N) # Number of full rounds of play
...
InitialDeclarations <-
  c("EvenSplit") # Initial declarations of the population, on of:
# 1. EvenSplit
# 2. UniformlyAtRandom
...
if (InitialDeclarations == "EvenSplit") {
  NetworkChoices <-
    sample(c(rep(1, floor(N / 2)), rep(0, ceiling(N / 2))))
  InitialState <- .5
}
# Uniformly at random
...
Prior <-
  InitialPrior <- c(0.5) # initial ignorance prior for all agents
...
BR <- function(FocalAgent, Signal) {
  ...
  EUvector <-
    c(EU(FocalAgent, 0, Signal), EU(FocalAgent, 1, Signal))
...
PublicPrior <- function(FocalAgent, Declaration) {
  ...
  ifelse (Isolated == TRUE,
          # If the focal agent is indeed isolated,
          Ns <-
            1 / 2,
          # then let her neighbors be 'tied' in their declarations, so only her truth-seeking payoff counts
          Ns <-
            CoordinationPayoff(FocalAgent, 1)) # If the focal agent is not isolated,
...
PublicBeliefActionPlot <-
  data.frame(
    0:NumberOfTurns,
    c(InitialPrior, PublicBelief),
    c(InitialState, PublicDeclarations)
  )
colnames(PublicBeliefActionPlot) <-
  c("Turn", "PublicBelief", "PublicDeclarations")
